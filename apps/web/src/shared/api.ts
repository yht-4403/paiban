...
export type Document = { id: string; unit_id: string; title: string; body: string; created_at: string; version: number; scope: 'private' | 'team' | 'round'; kind: 'note' | 'collection' | 'brief' | 'proposal' | 'decision'; round_id: string; refs: ResourceRef[] };
...
export type Message = { id: string; conversation_id: string; from_kind: 'human' | 'agent' | 'system'; from_unit: string; body: string; sources: string[]; created_at: string; meta: {
  mode?: string; status?: 'queued' | 'running' | 'done' | 'error' | 'cancelled'; run_id?: string; model?: string; error?: string; finish_reason?: string; duration_ms?: number; usage?: { total_tokens?: number; reasoning_tokens?: number };
  reasoning_effort?: ReasoningEffort; phase?: 'connecting' | 'thinking' | 'reading' | 'answering'; citations?: ResourceRef[];
} };
export type Task = { id: string; title: string; detail: string; status: string; assignee_id: string; creator_id: string; thread_id: string; priority: 'high'|'normal'|'low' };
export type State = { me: string; members: Member[]; threads: Thread[]; tasks: Task[]; documents: Document[];
...
export type AuthStatus = { needs_setup: boolean; workspace: string };
...
export const statusLabels: Record<Thread['status'], string> = { agent: 'Agent 通道', scheduled: '等待送达', waiting: '待本人处理', human: '本人已回复', resolved: '已确认' };
