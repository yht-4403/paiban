...
import { completionRetryAction, tutorialSnapshotOnPageLoad } from './tutorialRuntime';
import './tutorial.css';
...
export function TutorialController({ startSignal, state, data, view, routeId, busy, onNavigate, onOpenTrialChat, onRefresh, onContext, onSwitchAccount, onTask }: Props) {
  const [session, setSession] = useState<TutorialSession | null>(() => { tutorialSnapshotOnPageLoad(sessionStorage, STORAGE_KEY); return null; });
  ...
  const pause = () => { setPointer(null); releasePage(); setVisible(false); replaceSession(null); };
  ...
  const prepareCompletionWork = async (memberId: CompletionMember, taskId: string, forceNewThread = false) => {
    ...
    let threadId = !forceNewThread && current ? completionThreadId(current, memberId) : '';
  ...
  const advanceAfterCompletion = async (memberId: CompletionMember, current: TutorialSession) => {
    if (transitioning.current) return;
    transitioning.current = true;
    try {
      await onRefresh();
      if (memberId === TRIAL_TWO) {
        await prepareCompletionWork(TRIAL_THREE, current.taskThreeId);
      } else {
        await onSwitchAccount(TRIAL_ONE);
        onNavigate('meetings', current.meetingFlowId);
        await onRefresh();
        update({ phase: 'followup-ready', completionFlowId: '', error: '' });
      }
  ...
  const retryCompletion = async (memberId: CompletionMember) => {
    ...
    if (!current.completionFlowId) {
      await api('/tutorial/prepare', {});
      const next = await onSwitchAccount(memberId);
      const task = next.tasks.find(item => item.id === completionTaskId(current, memberId) && item.assignee_id === memberId);
      const threadId = completionThreadId(current, memberId);
      if (task && threadId) {
        try {
          const thread = await api<ThreadData>(`/threads/${threadId}`);
          const completed = thread.messages.some(message => message.conversation_id === threadId && message.from_kind === 'agent' && message.meta.status === 'done' && message.body.trim() && !current.completionBaseline.includes(message.id));
          if (completed) {
            onNavigate('workspace', threadId); onContext(true); await onRefresh();
            update({ phase: completionPhase(memberId, 'ready'), error: '' }); return;
          }
        } catch { /* Fall back to rebuilding the work step below. */ }
      }
      await prepareCompletionWork(memberId, completionTaskId(current, memberId)); return;
    }
    const flow = await api<FlowDetail>(`/flows/${current.completionFlowId}`);
    const action = completionRetryAction(flow.status, flow.error);
    if (action === 'advance') { await advanceAfterCompletion(memberId, current); return; }
    if (action === 'fresh-thread') {
      if (flow.status !== 'cancelled') await command(`/task-summaries/${flow.id}/cancel`, {});
      update({ completionFlowId: '' });
      await prepareCompletionWork(memberId, completionTaskId(current, memberId), true); return;
    }
    if (action === 'retry') await command(`/task-summaries/${flow.id}/retry`, {});
    else if (action === 'ready') { update({ phase: completionPhase(memberId, 'ready'), completionFlowId: '', error: '' }); return; }
  ...
  useEffect(() => { if (!startSignal || startSignal === lastStart.current) return; lastStart.current = startSignal; void prepare(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [startSignal]);
  ...
  useEffect(() => {
    ...
    const completionRecovery = phase === 'error' && session.recovery === 'completion-two'
      ? TRIAL_TWO
      : phase === 'error' && session.recovery === 'completion-three'
        ? TRIAL_THREE
        : null;
    const flowId = phase === 'meeting-prepare-wait' || phase === 'meeting-summary-wait' ? session.meetingFlowId : phase === 'completion-two-wait' || phase === 'completion-three-wait' || completionRecovery ? session.completionFlowId : phase === 'followup-wait' ? session.nextFlowId : '';
    if (!flowId) return;
    let stopped = false;
    const check = async () => {
      try {
        const flow = await api<FlowDetail>(`/flows/${flowId}`); if (stopped || transitioning.current) return;
        if (completionRecovery && flow.status === 'closed') {
          await advanceAfterCompletion(completionRecovery, session);
          return;
        }
        ...
        if ((phase === 'completion-two-wait' || phase === 'completion-three-wait') && flow.status === 'closed') {
          await advanceAfterCompletion(phase === 'completion-two-wait' ? TRIAL_TWO : TRIAL_THREE, session);
          return;
        }
        ...
        if ((phase === 'completion-two-wait' || phase === 'completion-three-wait') && flow.status === 'needs_input') { fail(new Error('当前对话还缺少可核验的完成结果。重试会新建一轮工作对话并重新生成，不需要回复 OK。'), phase === 'completion-two-wait' ? 'completion-two' : 'completion-three', phaseStep(session)); return; }
  ...
  /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [session?.phase, session?.recovery, session?.meetingFlowId, session?.completionFlowId, session?.nextFlowId]);
  ...
  return <>
    ...
    >
      ...
      ><span>步骤 {phaseStep(session)}/7</span><span className="tutorial-panel-actions"><button type="button" disabled={acting} onClick={() => void prepare()}>重新开始</button><button type="button" onClick={pause}>退出</button></span></div>
