import { TaskPriority } from '../chat/TaskPriority';
...
import { timeLabel, type Message, type ProcessAttachmentInput, type State, type Thread } from '../../shared/api';
...
export function ConversationInput({ thread, messages, state, onAction, composer }: {
  ...
  const handoffReady = !composer.running && messages.some(message => message.conversation_id === thread?.id && message.from_kind === 'agent'
    && message.from_unit === thread?.target_id && message.meta.status === 'done' && message.meta.finish_reason === 'stop' && message.body.trim());
  ...
  const current = action === 'handoff' && canHandoff && handoffReady ? action : action === 'confirm' && canConfirm ? action : null;
  ...
  const send = async (body: string, sources: string[], attachments?: ProcessAttachmentInput[]) => {
    setError('');
    if (!current || !thread) return composer.onSend(body, sources, attachments);
  ...
  return <div className="conversation-input" onKeyDown={event => { if (event.key === 'Escape' && current && !composer.busy) { setAction(null); setError(''); } }}>
    {!current && (canHandoff || canConfirm) && <div className="conversation-next-action">
      {canHandoff ? <Button variant="ghost" size="sm" disabled={composer.busy || !handoffReady} title={!handoffReady ? 'Agent 完整回答后可找本人' : undefined} onClick={() => start('handoff')}><UserLinedIcon />找本人</Button>
        : <Button variant="ghost" size="sm" disabled={composer.busy} onClick={() => start('confirm')}><CheckIcon />承接任务</Button>}
    </div>}
...
export function ConversationTask({ task, state, busy, onAction }: {
  ...
  return <div className={`conversation-task ${done ? 'task-completed' : ''}`}>
    <CheckIcon size={17} /><div><strong>{task.title}</strong><span>{owner?.person_name}负责 · {done ? '已完成' : '进行中'}</span></div>
    <TaskPriority task={task} editable={task.assignee_id===state.me} busy={busy} onChange={priority=>void onAction(`/tasks/${task.id}/priority`,{priority})} />
