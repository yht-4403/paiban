...
import { ExplorationTodoCard } from '../topics/ExplorationTodoCard';
import { contextTaskBuckets } from './contextTasks';
import './context-tree.css';

export function ContextPanel({ state, data, busy, onUse, onTask, onDeleteTask, onRefresh, onThread, onFlow, onTopic }: {
  state: State; data: ThreadData | null; busy: boolean; onDocument: (doc: ResourceRef) => void;
  onUse: (id: string) => void; onTask:(task:Task)=>void;onDeleteTask:(task:Task)=>void;onRefresh:()=>Promise<void>;onThread:(id:string)=>void;onFlow:(id:string)=>void;onTopic:(id:string)=>void;
}) {
  ...
  const {primary:unfinished,completedToday:done}=contextTaskBuckets(tasks);
