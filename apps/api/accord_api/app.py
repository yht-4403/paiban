...
def handoff(tid: str, body: Handoff, uid=Depends(principal)):
    def run(db):
        ...
        references=[]
        for message in db.execute('SELECT sources,meta FROM messages WHERE conversation_id=?',(tid,)):
            citations={ref['id']:ref for ref in json.loads(message['meta']).get('citations',[])}
            references.extend(citations.get(rid,{'id':rid}) for rid in json.loads(message['sources']))
