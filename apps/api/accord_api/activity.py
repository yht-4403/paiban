...
def visible(db,viewer,subject):
    ...
    result={'label':label,'source':'本人设置','seen_at':None,'agent_working':False,'work':None,'meeting':'未接入会议状态','shared_tasks':[],'permission_version':pref['version']}
    if pref['automatic']:
        result.update(label=('在 Accord 聊天' if presence['surface']=='chat' else '在 Accord 工作') if presence else label,
    ...
    for row in db.execute('''SELECT t.*,a.thread_id,a.creator_id FROM tasks t JOIN accord_task_acl a ON a.task_id=t.id
        WHERE t.assignee_id=? AND (t.assignee_id=? OR a.creator_id=?) ORDER BY t.created_at DESC LIMIT 20''',(subject,viewer,viewer)):
