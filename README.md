...
给小型项目团队的协作工具：问同事一件事，先由他的 AI 助手查资料回答；需要本人决定，再在同一段对话里接着聊，确认后变成待办。
...
「接口今天能联调吗？」一句简单的问题，常常变成等回复、反复催问，甚至临时拉会。我们希望先让 Agent 把信息准备好，再把需要人判断的事交给人。
...
| 真实 AI 问答 | DeepSeek 查阅授权资料，显示来源和用量，支持停止、重试 |
...
![团队分工：堡天负责产品与商业化，建城负责架构与工程，书傲负责产品体验，盛弘负责融资与资本规划](docs/参赛材料/图片/05-PPT团队分工.png)
...
打开 <http://127.0.0.1:5186> 创建工作空间，再邀请同事加入。数据保存在 `.local/workspace`。macOS 也可用 `python3 tools/dev-services.py start` 托管服务，用 `python3 tools/preview-services.py start --lan` 开放局域网访问。

- [后端测试](apps/api/tests) · [第三方与团队代码来源](THIRD_PARTY_NOTICES.md)
